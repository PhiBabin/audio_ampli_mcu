...
void draw_string_fast(const char* str, const uint32_t start_x, const uint32_t start_y, const uint32_t end_x, const LvFontWrapper& font, bool is_white_on_black, bool clear_side)
{
  if (str == NULL || *str == '\0')
  {
    return;
...
LvFontWrapper::LvFontWrapper(const lv_font_t* font) : font_(font), height_px_(font_->h_px - font_->h_top_skip_px - font_->h_bot_skip_px)
{
